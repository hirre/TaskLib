...
namespace TaskLibrary
{
    ...
    public static class TaskExtensions
    {
        ...
        /// <param name="handler">The timeout action</param>
        /// <param name="tokenSource">Cancellation token source</param>        
        public static void CreateTimeout(TimeSpan timeoutTime, Action handler,
            CancellationTokenSource tokenSource = null)
        {
            ...
            if (handler != null)
                Task.Delay(timeoutTime, tokenSource.Token)
                    .ContinueWith((task, o) => handler.Invoke(), tokenSource.Token);
        }
            ...
            CancellationTokenSource cancellationTokenSourceAction = null)
        {
            ...
            using (timeoutCancellationTokenSource)
            {
                ...
                if (await Task.WhenAny(task, delayedTask) == task)
                {
                    ...
                    return await task; // For propagating exceptions
                }
            ...
            CancellationTokenSource cancellationTokenSourceAction = null)
        {
            ...
            using (timeoutCancellationTokenSource)
            {
                ...
                {
                    timeoutCancellationTokenSource.Cancel();
                    await task; // For propagating exceptions
                }
                else
                {
                    // Timeout occured, invoke handler if there is any
